...
// ---- Object storage abstraction (Replit in prod, in-memory locally) ----

type ObjectStorageLike = {
  uploadFromBytes: (key: string, data: Buffer | Uint8Array) => Promise<void>;
  downloadAsBytes: (key: string) => Promise<Buffer | Uint8Array>;
};

let objectStorageClient: ObjectStorageLike;

if (process.env.REPL_ID) {
  // Running on Replit – use real object storage
  objectStorageClient = new Client({
    bucketId: process.env.DEFAULT_OBJECT_STORAGE_BUCKET_ID,
  }) as unknown as ObjectStorageLike;
  console.log("Using Replit object storage client.");
} else {
  // Local dev – simple in-memory stub to avoid sidecar/port 1106
  const inMemoryStore = new Map<string, Buffer>();

  objectStorageClient = {
    async uploadFromBytes(key, data) {
      const buf = Buffer.isBuffer(data)
        ? data
        : Buffer.from(data as Uint8Array);
      inMemoryStore.set(key, buf);
      console.log(`[local-storage] Stored ${key} (${buf.length} bytes)`);
    },
    async downloadAsBytes(key) {
      const buf = inMemoryStore.get(key);
      if (!buf) {
        throw new Error(`[local-storage] No file found for key: ${key}`);
      }
      console.log(`[local-storage] Retrieved ${key} (${buf.length} bytes)`);
      return buf;
    },
  };
  console.log("Using in-memory object storage for local development.");
}
...
// Authentication bypass - only for development/testing
// Set BYPASS_AUTH=true in .env to bypass authentication in development
const BYPASS_AUTH =
...
function requireAuth(
  ...
  if (BYPASS_AUTH) {
    // In bypass mode, create a mock user ID if not present
    if (!req.session.userId) {
      req.session.userId = "bypass-user-id";
    }
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (
      ...
      cb(
        new Error("Invalid file type. Only PDF, DOCX, and XLSX files are allowed."),
      );
    }
  },
});

// NOTE: original Replit-specific initialization removed;
// we now use the objectStorageClient defined above.
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/vendors", async (req: Request, res: Response) => {
    try {
      ...
      if (user.role !== "vendor") {
        return res
          .status(403)
          .json({ message: "Only vendors can create vendor profiles" });
      }
  ...
  app.get("/api/vendors/me", async (req: Request, res: Response) => {
    try {
      ...
      const vendor = await storage.getVendorByUserId(auth.userId);
      if (!vendor) {
  ...
  app.get("/api/vendors", async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Not authorized to view all vendors" });
      }
  ...
  app.get("/api/vendors/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Not authorized to view vendor details" });
      }
  ...
  app.post("/api/products", async (req: Request, res: Response) => {
    try {
      ...
      if (!vendor) {
        return res
          .status(403)
          .json({ message: "Vendor profile required to create products" });
      }
      ...
      if (req.body.skuNumber) {
        ...
        if (existingProduct) {
          return res
            .status(409)
            .json({ message: "SKU number already exists" });
        }
      }
  ...
  app.get("/api/products/pending", async (req: Request, res: Response) => {
    try {
      ...
      if (user.role !== "distributor" && user.role !== "auditor") {
        return res
          .status(403)
          .json({ message: "Not authorized to view pending products" });
      }
  ...
  app.post("/api/products/:id/submit", async (req: Request, res: Response) => {
    try {
      ...
      if (user.role !== "vendor") {
        return res
          .status(403)
          .json({ message: "Only vendors can submit products" });
      }
  ...
  app.post("/api/products/:id/review", async (req: Request, res: Response) => {
    try {
      ...
      if (user.role !== "distributor" && user.role !== "auditor") {
        return res
          .status(403)
          .json({ message: "Not authorized to review products" });
      }
      ...
      await storage.createAuditLog({
        ...
        entityType: "product",
        entityId: productId,
        description: `Product ${product.skuNumber} ${
          action === "approve" ? "approved" : "rejected"
        } by ${user.name} (${user.role})${notes ? `: ${notes}` : ""}`,
        changes: JSON.stringify(changes),
        version: product.version,
        ipAddress: req.ip,
      });
  ...
  app.get(
    "/api/products/:id/review-history",
    async (req: Request, res: Response) => {
      try {
        ...
        // Check authorization
        const isDistributorOrAuditor =
          user.role === "distributor" || user.role === "auditor";
        ...
        if (!isDistributorOrAuditor) {
          ...
          if (user.role === "vendor") {
            ...
            return res
              .status(403)
              .json({ message: "Not authorized to view review history" });
          }
        }

        // Get audit logs for this product
        const auditLogs = await storage.getAuditLogsByEntity(
          "product",
          productId,
        );

        // Filter to only review-related actions (approve, reject)
        const reviewHistory = auditLogs.filter(
          (log) => log.action === "approve" || log.action === "reject",
        );
  ...
  app.get("/api/products", async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Not authorized to view all products" });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.role !== "vendor") {
          return res
            .status(403)
            .json({ message: "Only vendors can upload documents" });
        }
  ...
  app.get("/api/documents/:id/download", async (req: Request, res: Response) => {
    try {
      ...
      const fileBuffer = await objectStorageClient.downloadAsBytes(
        document.storageKey,
      );
  ...
  app.post("/api/documents/:id/sign", async (req: Request, res: Response) => {
    try {
      ...
      if (!canSign) {
        return res
          .status(403)
          .json({ message: "Not authorized to sign documents" });
      }
      ...
      const validation =
        insertDigitalSignatureSchema.safeParse(signatureRecord);
      if (!validation.success) {
        return res.status(400).json({
          message: fromZodError(validation.error).message,
        });
  ...
  app.get("/api/audit/logs", async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Not authorized to view audit logs" });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (user.role === "vendor") {
          ...
          if (!vendor || product.vendorId !== vendor.id) {
            return res.status(403).json({
              message: "Not authorized to view this product's audit trail",
            });
          }
        }
